  ...
  <div class="app-container">
    ...
    <div class="box-body">
      ...
      >
        ...
        <el-table-column prop="r_num" label="注册活跃" align="center"></el-table-column>
        <el-table-column prop="s_num2" label="次日留存" align="center">
          <template slot="header" slot-scope="scope">
            次日留存
            <el-tooltip class="item" content="次日留存率 =  次日依然活跃的用户 / 今日注册活跃人数" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
          </template>
          <template slot-scope="scope">
            <span v-if="scope.row.s_num2 == 0">0%</span>
            <span v-else="scope.row.s_num2 != 0">{{scope.row.s_num2 / scope.row.r_num *100 | numFilter}}%</span>
          </template>
        </el-table-column>
        <el-table-column prop="s_num3" label="3日留存" align="center">
          <template slot="header" slot-scope="scope">
            3日留存
            <el-tooltip class="item" content="3日留存率 =  第3日依然活跃的用户 / 今日注册活跃人数" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
          </template>
          <template slot-scope="scope">
            <span v-if="scope.row.s_num3 == 0">0%</span>
            <span v-else="scope.row.s_num2 != 0">{{scope.row.s_num3 / scope.row.r_num * 100 | numFilter}}%</span>
          </template>
        </el-table-column>
        <el-table-column prop="s_num7" label="7日留存" align="center">
          <template slot="header" slot-scope="scope">
            7日留存
            <el-tooltip class="item" content="7日留存率 =  第7日依然活跃的用户 / 今日注册活跃人数" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
          </template>
          <template slot-scope="scope">
            <span v-if="scope.row.s_num7 == 0">0%</span>
            <span v-else="scope.row.s_num2 != 0">{{scope.row.s_num7 / scope.row.r_num * 100 | numFilter}}%</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import { dailystay } from "@/api/dailydata";
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      dailystay(this.params).then(res => {
